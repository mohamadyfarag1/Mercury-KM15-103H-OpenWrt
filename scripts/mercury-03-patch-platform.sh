...
cp ../mercury_km15_103h_build/files/lib/upgrade/platform.sh package/base-files/files/lib/upgrade/platform.sh
...
if [ -f "$PLATFORM_SH" ]; then
    if ! grep -q "mercury,km15-103h" "$PLATFORM_SH"; then
        echo "Patching $PLATFORM_SH to hook nand_do_upgrade..."
        python3 -c '
import re, sys
path = sys.argv[1]
...
if "mercury,km15-103h" not in content:
    if "nand_do_upgrade" in content:
        content = content.replace("nand_do_upgrade \"$1\"", "nand_do_upgrade \"$1\"\n" + target_block)
    elif "default_do_upgrade" in content:
